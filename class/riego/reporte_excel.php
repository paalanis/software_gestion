...
/*Extraer datos de MYSQL*/
    ...
    $cauda=$_POST['post_cauda'];
	$desde=$_POST['post_desde'];
	$hasta=$_POST['post_hasta'];
	$finca=$_POST['post_finca'];
	$consulta_cauda = "";
	if ($cauda != "") {
	$consulta_cauda = "AND tb_caudalimetro.id_caudalimetro = $cauda";
	}
	...
	$sqlriego = "SELECT
		...
		WHERE
		tb_milimetros_riego.fecha BETWEEN '$desde' AND '$hasta' AND tb_finca.id_finca = '$finca' $consulta_cauda
		ORDER BY
		fecha asc";
...
header('Content-Disposition: attachment;filename="reporte_riego.xls"');
